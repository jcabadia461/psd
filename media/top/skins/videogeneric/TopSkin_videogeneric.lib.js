...
(function(namespace){
    ...
    TopSkin_videogeneric.prototype = new emic.top.ui.UISkinBase();

    function TopSkin_videogeneric (){
        ...
        var _wrapper,               // content ui layer
            ...
            _relatedVideos,         // spam where on create class "videogeneric_enlaces_videos" with DHTML
        ...
        var
            ...
            _claseActual;           //  = "videogeneric_estado_pausado";
        ...
        var _URL_TEMPLATE = "/psdmedia/media/top/skins/videogeneric/{UDN}/assets/template.html",   // template
            _URL_STYLE = "/psdmedia/media/top/skins/videogeneric/{UDN}/assets/estilos.css",        // main style
            _URL_STYLE2 = "/psdmedia/media/top/skins/videogeneric/{UDN}/assets/estilos_ie8.css",   // IE8 style
            _URL_JS = "/psdmedia/media/top/skins/videogeneric/motor/assets/js/",
        ...
        /*        var _callODB = function() {
         ...
         permalink:"http://www.huff.com/448267",      // url from
         widgetId: "APP_1"
         };
        ...
        var quitarBarraInferior= function(){
            clearTimeout(miTimeout);
            miTimeout = setTimeout(function(){
                _barraInferior.style.display= "none";
            },3000);
        };

        var mostrarBarraInferior= function(){
            _barraInferior.style.display= "block";
        };
        ...
        var _loadCSS2 = function(){                             // Loading CSS secondary file
            ...
            _URL_STYLE2 = _URL_STYLE2.replace("{UDN}",_udn);
        ...
        this.onSwitchRequest = function(id) {
            ...
            _wrapper.className = _wrapper.className + " videogeneric_cargando_calidad";
        };
        ...
        this.onSwitchComplete = function(id) {
            ...
            for(j in aelements) {
                if (j == id) {
                    aelements[j].className = "videogeneric_seleccionado";
                }
                else
        ...
        var _createRendition = function(rendition, index){      // It's used in showQuality to control option clicked...
            ...
            aelement.onclick = (function(that){
                return function(e){
                    aelement.className = "videogeneric_seleccionado";
        ...
        var showRelatedVideos = function(relatedVideos) {
            ...
            if (n > 0) {                        // there is a thumbnail at least
                ...
                for (var i = 1; i <= rows; i++) {
                    ...
                    contenido.className = "videogeneric_enlaces_videos videogeneric_estirar";
        ...
        var _loadVideos = function() {
            ...
            //_data.mediaData.relacionadosUrl = "http://fapi-top.prisasd.com/api/search/huff/video/?order=true&size=12";
        ...
        var _asignElements = function(){
            ...
            _duration = 0;
            _isIE8 = false;
            _gotolive  = _getElementById("gotolive_");
            _contenedor_video = _getElementById("contenedor_video_");
            _barraInferior= _getElementById("barrainferior");

            if((psd.framework.ua.msie!=null)&&(psd.framework.ua.msie)&&(parseInt(psd.framework.ua.version)<11)){
                //_fullScr.style.display = "none";
                _wrapper.className =  _wrapper.className + " videogeneric_b_fullscreen_0";
            }

            _contenedor_video.ondblclick = (function(that){
                return function(){
                    if (_isFullScreen)
                        _isFullScreen = false;
                    else
            ...
            _mute.onclick = (function(that){
                return function(e){
                    if(!_muted)
                        ...
                        that.notifyOrderVolumeChange(_currentVolume);
            ...
            _playShare.onclick = (function(that) {
                return function() {
                    ...
                    changeGenericStyle(" videogeneric_estado_compartiendo");
                }
            })(this);
            ...
            _cerrarCompartir.onclick = (function(that) {
                return function() {
                    ...
                    changeGenericStyle(" videogeneric_b_compartir_0");
                    if ((_flag) && (_ended)) {                                  // this is the end
                        changeGenericStyle(" videogeneric_estado_fin videogeneric_b_calidad_0");
                        //changeGenericStyle(" videogeneric_estado_fin");
                    } else {
                        _changeToMediaClass();
            ...
            _gotolive.onclick = (function(that){
                return function(){
                    //if (getDevice().mobile){that.notifyOrderSeekLive();}
                    //else{that.notifyOrderReset(_data);}
                    that.notifyOrderSeekLive();
                    _gotolive.className = _gotolive.className.replace(/videogeneric_parado/g,"");
                    //that.notifyOrderSeekBySecs(-1);
                }
            })(this);

            switch (_data.uiData.skinData.previewMode){
                case "small":
                    _claseInicial = " videogeneric_estado_off_2";
                    _wrapper.className += _claseInicial;
                    _claseActual = _claseInicial;
                    break;
                default:
                    _claseInicial = " videogeneric_estado_off_1";
                    _wrapper.className += " videogeneric_estado_off_1";
                    _claseActual = _claseInicial;
                    break;
            ...
            if (getDevice().mobile){
                _wrapper.onclick = (function(that){
                    return function(){
        ...
        {
            //funcion necesaria para que funcione el reset ttt
            _imagenInicio.style.visibility = 'visible';
            _reproducido.style.width = 0;
            _manejador.style.left = 0;

            //document.getElementsByClassName("videogeneric_modulo_datos")[0].style.display = "none";
            ...
            if (_data.mediaData.renditions.length > 0) {
                ...
                _wrapper.className =  _wrapper.className + " videogeneric_b_calidad_0";
                _flag = false;
            }
            ...
            if (getDevice().mobile)
            {
                if (getDevice().agent.toLowerCase().indexOf('android') == -1) _fullScr.style.display = "none";
                document.getElementsByClassName("videogeneric_volumen")[0].style.display = "none";
            ...
            _progressBar.onclick = (function(that, __currentTarget){
                return function(e){
                    ...
                    _gotolive.className += " videogeneric_parado";

                    if((_data.mediaData.isLiveActive)&&(!_data.mediaData.isDVR)){}
                    else{
                        var x, y, parentPosition;
                        var ct = ((psd.framework.ua.msie!=null)&&(psd.framework.ua.msie)&&(parseInt(psd.framework.ua.version)<9))? __currentTarget: e.currentTarget;
                        var evento = e || window.event;
        ...
        this.init = function(data){
            ...
            _via = _data.uiData.skinData.via;
        ...
        var _showLoading = function(flag){
            ...
            if(!inValid && (_data.internalData.position == emic.top.TopPlayer.POSITION_MEDIA))
                ...
                if (flag) {
                    ...
                    changeGenericStyle(" videogeneric_estado_cargando videogeneric_cargando_calidad");
                    //_progressBar.style.display = "none";
        ...
        this.onStatusChange = function(status) {
            switch(status){
                ...
                case emic.top.MediaModule.STATUS_PLAY:
                    ...
                    if ((psd.framework.ua.msie!=null)&&(psd.framework.ua.msie)&&(parseInt(psd.framework.ua.version)<9)){
                        changeGenericStyle(" videogeneric_estado_on videogeneric_b_compartir_0");
                    }
                ...
                case emic.top.AdModule.STATUS_PLAY:
                    ...
                    changeGenericStyle(" videogeneric_estado_on videogeneric_b_calidad_0 videogeneric_b_compartir_0");

                    break;
                case emic.top.AdModule.STATUS_PAUSE:
                    _totalTime.style.visibility = "visible";
                    changeGenericStyle(" videogeneric_estado_pausado videogeneric_b_calidad_0 videogeneric_b_compartir_0");
                ...
                case emic.top.AdModule.STATUS_STOP:
                    ...
                    _thumbnail.style.visibility = "visible";
					_imagenInicio.style.visibility = "hidden";
                    //changeGenericStyle(" videogeneric_estado_on");
                    break;
            ...
            switch(position) {
                ...
                case emic.top.TopPlayer.POSITION_PREVIEW:
                    ...
                    if (_flag)  {
                        ...
                        changeGenericStyle(_claseInicial + " videogeneric_b_calidad_0");
                        if (_isThereRenditions)
        ...
        {
            if (_data.mediaData.isLiveActive){_gotolive.style.visibility = "visible";}
            else {_gotolive.style.visibility = "hidden";}

            if ((getDevice().mobile) && (_data.mediaData.isLiveActive == true)){_progressBar.style.display = "none";}
            else {_progressBar.style.display = "block";}
        ...
        this.onVolumeChange = function(offset){
            if(_mascaravolumen != undefined){_mascaravolumen.style.height = (offset*100) + "%";}
            if(offset!=0){
        ...
        var _changeToMediaClass = function(){
            if (_data.internalData.position == emic.top.TopPlayer.POSITION_MEDIA){
                if (_isPaused) {
                    changeGenericStyle(" videogeneric_estado_pausado");
                } else{
                    changeGenericStyle(" videogeneric_estado_on");
                }
            }
            ...
            if((_data.internalData.position==emic.top.TopPlayer.POSITION_AD_PREROLL)||(_data.internalData.position==emic.top.TopPlayer.POSITION_AD_POSTROLL))
                ...
                changeGenericStyle(" videogeneric_estado_off_1");
        }
    }
    namespace.TopSkin_videogeneric = TopSkin_videogeneric;
