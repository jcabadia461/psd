...
var s_account="prisacomhuffingtonpost"
var s_accountF="prisacomhuffingtonpost"
...
var first_dot = site.indexOf(".", 1);
var last_dot = site.lastIndexOf(".");

/* Page Name Config */
s.siteID = site.slice(first_dot + 1);										
s.defaultPage="";
s.queryVarsList="";
s.pathExcludeDelim=";";
s.pathConcatDelim="/";
...
if (typeof(subseccion) == 'undefined') subseccion=''; 
if (typeof(subsubseccion) == 'undefined') subsubseccion=''; 
if (typeof(type) == 'undefined') type='otros'; 
if (typeof(userId) == 'undefined')userId='';reg="anónimo";if (userId!=''){reg="logueado"}
...
/************** End doPlugins Script **************/

/* Example add events 
	s.events=s.apl(s.events,'event1',1)
*/

	// s.pageName+= ':' + document.title;															// Manual formation s.pageName (off)

	s.pageName=s.siteID + location.pathname;														// uncomment for use Plugin getPageName() 
			
	s.prop5="D=g";							  														// URL 	  									
	s.prop6="D=r";									   												// Referrer 								
	s.prop14="españa";											    								// Country 						
	s.prop17="web";																					// Canal 									
	s.prop18="prisa";																				// Organization
	s.prop19="huffingtonpost";																		// Producto
	s.prop20="www.huffingtonpost.es";																// Domain | Subdomain																		
	s.prop30="noticias";																			// Bussines Unit		
	s.prop31="informacion"; 																		// Temathic																
	s.prop45="D=v8"																					// Title / Page Name				
	s.prop62=reg;																					// Log in / Anonymous		

	
// Set Page View Event 
	s.events=s.events?s.events+',event2':'event2';

// force all Adobe SiteCatalyst variables toLowerCase 
...
// Exclude props toLowerCase / Conversion variables 
	...
	if(s.prop22)s.eVar24="D=c22"	// Format	    											
...
// function launch Ajax
function launchAjaxOMN(eVars,eVars_value,evento){
	s.usePlugins=false
	s=s_gi(s_accountF)
	var AeVars = eVars.split("|");
	var AeVars_value = eVars_value.split("|");
	s.linkTrackVars='events,eVar3,eVar10,eVar43';
	...
	s.eVar3=s.getPageName(); 	// pageName
	s.eVar10="D=g"				// URL pep
...
+"th>0){for(i=s.d.forms.length-1;i>=0;i--){fo=s.d.forms[i];fn=fo.name?fo.name:fo.id"
...
+"form.name,en=this.name?this.name:this.id,sc=false;if(document.layers){kp=e.which;b=e."
...
s.channel=seccion;
s.pageName=s.channel;								// Manual formation s.PageName (Recommended Omniture SiteCatalyst)

if(subseccion != '' ) {
	s.prop1 = seccion + '>' + subseccion;			// subsección: "seccion>subseccion"
	s.pageName+= ':' + subseccion;					// Manual formation s.PageName
}else{ 
	s.prop1 = '';
}
if(subsubseccion != ''){
	s.prop2= s.prop1 + '>' +subsubseccion;			// subsubsección: "seccion>subseccion>subsubseccion"
	s.pageName+= ':' + subsubseccion;				// Manual formation s.PageName
}else{
	s.prop2 = '';
}

// Hierarchy WEBSITES GROUP: 
