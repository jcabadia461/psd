...
s.tagContainerDC="d1"
s.tagContainerNamespace="prisacom"
s.tagContainerName="CadenaSER"
s.loadTagContainer()

/* Función de seguimiento de redes sociales */
function funcionSeguimiento(accion,contenidoCompartido,valor){
	//Incluir código de medición asíncrono
	s=s_gi(s_account);
	s.linkTrackVars="eVar8,eVar28,prop39,eVar39,eVar69,events";
	s.linkTrackEvents="event69";	
	if (typeof(accion)=='object'){
		s.eVar69='google_plusone';
		}
		else {
			s.eVar69=accion; 
		}			
	if(contenidoCompartido=="pageTitle"){
		s.prop39=valor;
		s.eVar39=valor;			//Será pageTitle compartido
	}
	else if(contenidoCompartido=="video"){
		s.eVar8=valor;			//Será el nombre del vídeo compartido
	}
	else if (contenidoCompartido=="audio"){
		s.eVar28=valor;			//Será el nombre del audio compartido
	}
	s.events="event69";
	s.tl(this,"o","Accion boton-"+s.eVar69+"-"+contenidoCompartido);
	s.manageVars("clearVars");
}



/* Funciones para digilant */
...
function digilant(){
	if (typeof(code_digilant) == "undefined" )
	{
		...
		var codigoDigilant = '<html>\n<body>\n' +
			'<script language="JavaScript" type="text/javascript" src="http://www.wtp101.com/pixel?id=16302&pb=1100&rd=3&st=' + escape(getCookie('s_formProp20')) + '&se=' + escape(getCookie('s_formChannel')) + '&yr=&s=&ct=&ca=' + escape(getCookie('s_formProp30'))+escape(getCookie('s_formProp22')) +'"></scr' + 'ipt>\n'+
			'</body>\n</html>\n';
